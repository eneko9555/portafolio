...
const page = () => {
  ...
  async function getMainProjects() {
    try {
      const { data } = await axios(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/projects/main`
      )
      console.log(data);
      ...
      <main className='p-4 w-full sm:p-0 sm:w-3/4 md:w-8/12 lg:w-4/6 m-auto sm:mt-12 lg:mt-32 '>
        <div>
          ...
          >
            <h1 className='font-bold text-white tracking-wide text-5xl'>
              Proyectos principales. </h1>
          </motion.div>

          <div className=' grid grid-flow-row mb-20 grid-cols-1 xl:grid-cols-2 gap-6 mt-4 m-auto justify-center '>
        ...
        >
          <div className='flex  items-center  justify-between text-white'>
            <h2 className='font-bold text-white tracking-wide text-4xl'>
              Otros proyectos.
            </h2>
            ...
            <svg
              ...
              strokeWidth={1.5}
              stroke='currentColor'
              className='w-10 h-10 cursor-pointer hover:text-gray-400 duration-300 text-center '
              onClick={() =>
                limit.length <= 4
                  ? showAllProjects()
        ...
        <AnimatePresence>
          {all && limit.length > 4 && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -50 }}
            >
              <div className=' justify-center m-auto flex mt-4 gap-2 '>
                <button
                  type='button'
                  className={`text-white  text-lg  p-2 w-20  duration-150 border border-white ${category === ''
                ...
                <button
                  type='button'
                  className={`text-white  text-lg p-2  duration-150 border border-white ${category === 'fullstack'
                    ? '  font-black text-white border-4'
                    : ' font-normal '
                    }`}
                ...
                <button
                  type='button'
                  className={`text-white  text-lg p-2  duration-150 border border-white ${category === 'frontend'
                    ? ' font-black text-white border-4'
                    : ' font-normal'
                    }`}
        ...
        <div className=' grid grid-flow-row grid-cols-1 xl:grid-cols-2 gap-6 mt-4 m-auto justify-center '>
          {!category
            ? limit?.map((project) => (
              <Project
                project={project}
        ...
        {limit?.length <= 4 && (
          ...
          >
            <div
              className=' m-auto text-center flex items-center gap-3 mt-12 justify-center hover:text-gray-400 duration-300 bg-white rounded-lg p-3  text-black hover:cursor-pointer  '
